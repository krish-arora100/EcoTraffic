// Data analysis utility for min/max percent change. Not used in main app runtime.
// Utility to get min/max percent change from 2000 for all counties and years
import { promises as fs } from 'fs';
import path from 'path';

export async function getPercentChangeMinMax() {
  ...
  for (let i = headerIdx + 1; i < lines.length; i++) {
    ...
    if (row.length < Math.max(countyIdx, yearIdx, totalIdx) + 1) continue;
    if (row[yearIdx] && row[yearIdx].trim() === '2000') {
      const county = row[countyIdx].replaceAll('"', '').trim();
      const value = parseFloat(row[totalIdx]);
...
getPercentChangeMinMax().then(console.log);
