...
export default function ChartView() {
  ...
  useEffect(() => {
    // If there are counties in the query params, use them as initial selection
    if (searchParams) {
      const countiesFromParams = searchParams.getAll('counties');
      if (countiesFromParams.length > 0) {
        setSelectedCounties(countiesFromParams.filter(c => COUNTY_LIST.includes(c)));
      }
    }
  }, [searchParams]);
  ...
  return (
    <div style={{ width: "100%", height: "80vh", position: "relative", display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        {selectedCounties.length > 0 && (
          <div style={{marginBottom: 12, background: 'rgba(255,255,255,0.45)', borderRadius: 12, padding: '12px 14px', color: '#1e2a22', fontSize: 15, fontWeight: 500, boxShadow: '0 1px 4px #b6e38822'}}>
            {selectedCounties.map((county, idx) => (
              <div key={county} style={{marginBottom: 12, borderBottom: '1px solid #e0e0e0', paddingBottom: 8}}>
                <div style={{display:'flex', alignItems:'center', gap:8, marginBottom:4}}>
                  ...
                  <span style={{fontWeight:700, fontSize:18}}>{county} County</span>
                </div>
                <div style={{marginBottom:2, fontWeight:600, fontSize:15}}>Emissions by Year:</div>
                <div style={{display:'flex', flexDirection:'column', gap:2}}>
                  {allYears.map(year => {
                    const yearData = (data[county] || []).find(d => d.year === year);
                    return (
                      <div key={year} style={{display:'flex', justifyContent:'space-between', fontSize:15, fontWeight:500, color:'#1e2a22'}}>
                        <span style={{color:'#20632e', fontWeight:600}}>{year}:</span>
                        <span style={{color:COUNTY_COLORS[idx % COUNTY_COLORS.length], fontWeight:700}}>{yearData ? yearData.total.toLocaleString(undefined, { maximumFractionDigits: 0 }) + ' MT' : 'N/A'}</span>
                      </div>
                    );
                  })}
                </div>
      ...
      <div style={{
        ...
        <h2 style={{ textAlign: "center", color: "#1fa463", fontWeight: 800, fontSize: 24, marginBottom: 6, letterSpacing: 0.2, marginTop: 6 }}>{selectedCounties.length === 1 ? `${selectedCounties[0]} County Emissions Over Time` : `County Emissions Over Time`}</h2>
